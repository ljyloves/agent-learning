...
def add_memory_demo(memory_tool):
    ...
    # 强行传入一个虚拟的音频路径和 modality 模态，逼系统去初始化 CLAP
    audio_result = memory_tool.run(
        {
            "action": "add",
            "content": "这是一段会议录音的文本摘要",
            "memory_type": "perceptual",  # 知觉记忆
            "file_path": "meeting_record.mp3",  # 伪造一个音频后缀，触发内部的 _infer_modality 逻辑
            "modality": "audio",  # 明确指定是音频
            "importance": 0.5,
        }
    )
    print(f"音频模块触发状态: {audio_result}")
